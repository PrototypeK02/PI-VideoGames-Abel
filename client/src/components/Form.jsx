        ...
        switch (name) {
            case "name":
                if(value.length > 0) {
                  ...
                  return  setError({...error, name: "This field cannot be empty"})
                }
...
async function validateSubmit(e) {
    ...
    if(error.name || error.description || error.rating || error.releaseDate ) {
        e.preventDefault()
        console.log("Nice Try! ;)")
        throw alert("Nice Try!! ;)")
        ...
        try {
            e.preventDefault()
         await axios.post("http://localhost:3001/videogames", inputs)
  ...
  return (
    ...
    <div >
      ...
      {error.name && <span className='error'>{error.name}</span>}
        ...
        <input name="rating" className="formInputs2" placeholder='Rating...' value={inputs.rating} onChange={handleChange}></input>
        ...
        <button disabled={!error.name && !error.description && !error.rating ? false: true}className='btnForm' onClick={(e) => {return validateSubmit(e)}}>Create</button>
